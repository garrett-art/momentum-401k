...
const toTitleCase=str=>{
  ...
  return str.toLowerCase()
    ...
    .replace(/\bMd\b/gi,'MD')
    .replace(/\bDmd\b/gi,'DMD')
    .replace(/\bDds\b/gi,'DDS')
...
function CallLogger({plan,onSave,onCancel}){
  const [call,setCall]=useState({...BLANK_CALL,id:uid(),date:new Date().toISOString().split("T")[0]});
  const [showTranscript,setShowTranscript]=useState(false);
  const [processing,setProcessing]=useState(false);
  const set=f=>v=>setCall(p=>({...p,[f]:v}));
  const inp={width:"100%",padding:"7px 10px",border:`1px solid ${RULE}`,borderRadius:4,fontFamily:"inherit",fontSize:13,color:INK,background:"#fff",outline:"none"};

  const processTranscriptFn=async()=>{
    if(!call.rawTranscript)return;
    setProcessing(true);
    try{
      const res=await fetch("/api/transcript",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({transcript:call.rawTranscript,plan})});
      const data=await res.json();
      if(data&&!data.error) setCall(p=>({...p,summary:data}));
    }catch(e){console.error("Transcript error:",e);}
    finally{setProcessing(false);}
  };

  return(
    <div style={{background:FILL,border:`1px solid ${RULE}`,borderRadius:4,padding:16,marginBottom:16}}>
      <div style={{fontFamily:"'Syne',sans-serif",fontSize:11,fontWeight:700,color:BLUE,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:12}}>Log a call</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:10}}>
        <div>
          <label style={{display:"block",fontFamily:"'Syne',sans-serif",fontSize:9,fontWeight:700,color:MUTED,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Date</label>
          <input type="date" value={call.date} onChange={e=>set("date")(e.target.value)} style={inp}/>
        </div>
        <div>
          <label style={{display:"block",fontFamily:"'Syne',sans-serif",fontSize:9,fontWeight:700,color:MUTED,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Outcome</label>
          <select value={call.outcome} onChange={e=>set("outcome")(e.target.value)} style={{...inp,cursor:"pointer"}}>
            {Object.entries(CALL_OUTCOMES).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
          </select>
        </div>
        <div>
          <label style={{display:"block",fontFamily:"'Syne',sans-serif",fontSize:9,fontWeight:700,color:MUTED,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Duration</label>
          <input value={call.duration} onChange={e=>set("duration")(e.target.value)} placeholder="e.g. 12 min" style={inp}/>
        </div>
      </div>
      <div style={{marginBottom:10}}>
        <label style={{display:"block",fontFamily:"'Syne',sans-serif",fontSize:9,fontWeight:700,color:MUTED,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:4}}>Notes</label>
        <textarea value={call.notes} onChange={e=>set("notes")(e.target.value)} rows={3} placeholder="What happened on this call..." style={{...inp,resize:"vertical"}}/>
      </div>
      <div style={{marginBottom:12}}>
        <button onClick={()=>setShowTranscript(!showTranscript)} style={{background:"none",border:"none",cursor:"pointer",color:BLUE,fontSize:12,fontFamily:"inherit",padding:0}}>
          {showTranscript?"▲ Hide transcript":"▼ Paste transcript for AI summary"}
        </button>
        {showTranscript&&(
          <div style={{marginTop:8}}>
            <textarea value={call.rawTranscript||""} onChange={e=>set("rawTranscript")(e.target.value)} rows={5} placeholder="Paste call transcript here..." style={{...inp,resize:"vertical",marginBottom:6}}/>
            <Btn onClick={processTranscriptFn} disabled={processing||!call.rawTranscript} small variant="secondary" icon={processing?<Loader2 size={11} style={{animation:"spin 1s linear infinite"}}/>:undefined}>
              {processing?"Processing...":"Generate AI Summary"}
            </Btn>
            {call.summary&&<div style={{marginTop:8,padding:"8px 10px",background:"#fff",border:`1px solid ${RULE}`,borderRadius:4,fontSize:11,color:BODY}}>{call.summary.callSummary}</div>}
          </div>
        )}
      </div>
      <div style={{display:"flex",gap:8}}>
        <Btn onClick={()=>onSave(call)} icon={<Save size={12}/>}>Save Call</Btn>
...
function PlanDetail({plan,onBack,onUpdate,onEdit,settings}){
  ...
  const mark=(field,dateField)=>{
    ...
    onUpdate({...plan,status:nextStatus,outreach:{
      ...plan.outreach,
      [field]:true,
      [dateField]:plan.outreach[dateField]||today,
      [field+"By"]:plan.outreach[field+"By"]||byName
    }});
  };

  const liMsg=buildLinkedIn(plan);

  const Step=({num,label,done,date,by,onMark,copyContent,extra})=>(
    <div style={{display:"flex",alignItems:"flex-start",gap:12,padding:"10px 0",borderBottom:`1px solid ${RULE}`}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          {done&&date&&<span style={{fontSize:11,color:MUTED}}>{fmtDs(date)}</span>}{done&&by&&<span style={{fontSize:11,color:BLUE,fontWeight:500,marginLeft:4}}>{by}</span>}
        </div>
        {extra}
      </div>
      ...
      <div style={{display:"flex",gap:20}}>
        ...
        <div style={{flex:"1 1 0",minWidth:0,display:"flex",flexDirection:"column",gap:16}}>
          ...
          <Card>
            ...
            <Step num={1} label="Postcard mailed" done={plan.outreach?.postcardSent} date={plan.outreach?.postcardDate} by={plan.outreach?.postcardSentBy} onMark={()=>mark("postcardSent","postcardDate")}/>
            <Step num={2} label="LinkedIn connection request" done={plan.outreach?.linkedInSent} date={plan.outreach?.linkedInDate} by={plan.outreach?.linkedInSentBy} onMark={()=>mark("linkedInSent","linkedInDate")} copyContent={liMsg}
              extra={
...
function ProspectTab({plans,onAddPlans,targets,onTargetsChange,sizeFilter,onSizeFilterChange,results,onResultsChange}){
  ...
  const addToPipeline=()=>{
    const toAdd=filtered.filter(r=>selected.has(r.ein)).map(r=>({
      ...
      contactName:r.adminName?toTitleCase(r.adminName):"",
      ...
      calls:[],analysis:null,
      liVariant:Math.floor(Math.random()*5),
    }));
    onAddPlans(toAdd);
    setSelected(new Set());
  };
...
export default function App(){
  ...
  const loadUserData=async(userId)=>{
    try{
      const [p,s]=await Promise.all([
        dbLoadPlans().catch(e=>{console.error("plans load failed:",e);return [];}),
        dbLoadSettings(userId).catch(e=>{console.error("settings load failed:",e);return null;})
      ]);
      setPlans(p||[]);
      // Use Supabase settings, fall back to localStorage cache, then defaults
      let localSettings=null;
      try{const ls=localStorage.getItem("mw_settings");if(ls)localSettings=JSON.parse(ls);}catch(e){}
      setSettings(s||localSettings||DEFAULT_SETTINGS);
    }catch(e){
  ...
  const persistSettings=async s=>{
    setSettings(s);
    try{localStorage.setItem("mw_settings",JSON.stringify(s));}catch(e){}
    const uid=userIdRef.current;
    if(uid){try{await dbSaveSettings(s,uid);}catch(e){console.error("Save settings error:",e);}}
